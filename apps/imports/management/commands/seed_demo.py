...
class Command(BaseCommand):
    ...
    def _create_content(self, branch) -> int:
        ...
        # Номер счётчика намеренно очищается: он принадлежит заказчику и
        # действует на его сайте. Демо-стенд с ним слал бы хиты в чужую
        # аналитику. Именно очищается, а не пропускается: на стенде, засеянном
        # прежней версией, номер иначе остался бы в базе. На проде он вносится
        # через админку.
        settings_obj.metrika_id = ""
        ...
        pages = [
            ...
            (
                ...
                "Сайт использует сервис веб-аналитики Яндекс.Метрика для сбора "
                "обезличенной статистики посещений. Также сайт сохраняет в файле cookie "
                "метки рекламной кампании, по которой посетитель пришёл, — чтобы "
                "понимать, какие источники приводят обращения. Метки хранятся 30 дней и "
                "удаляются вместе с остальными данными заявки по истечении срока "
                "хранения.",
            ),
        ]
