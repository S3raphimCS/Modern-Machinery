...
Наполнение `seed_demo` заводит правила с адресами вида `sales@`, `parts@`,
`service@` — **на домене заказчика `modernmachinery.ru`**. Это заглушки для
показа маршрутизации, а не рабочие ящики. Перед тем как подключать реальную
почту, замените их своими, иначе письма уйдут на чужой домен.

Убрать адрес из всех правил:

```bash
docker compose -f docker-compose.prod.yml exec web \
  python manage.py remove_lead_email sales@modernmachinery.ru --dry-run
```

Без `--dry-run` изменения сохранятся. Само правило остаётся: оно продолжает
определять отдел заявки, а письмо уходит подписчикам с галочкой.

Резервный адрес задаётся отдельно, в `.env`:
...
Повторный запуск `seed_demo` списки адресов больше не перезаписывает — правки
