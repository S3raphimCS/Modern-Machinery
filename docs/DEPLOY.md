...
По умолчанию `EMAIL_URL=consolemail://` — письмо печатается в журнал контейнера.
Заявка сохраняется в базе и видна в админке, но менеджер о ней не узнает.
...
DEFAULT_FROM_EMAIL=login@domain.ru
```

Если 465-й порт у провайдера закрыт, тот же ящик через STARTTLS:
...
1. **Логин — полный адрес, собака пишется как `%40`.** Иначе строка
   подключения разберётся неправильно: всё после первой собаки будет принято
   за имя сервера.
2. **Пароль нужен отдельный, «для приложения».** Создаётся в Яндекс ID →
   Безопасность → Пароли приложений → Почта. Пароль от учётной записи Яндекс
   для SMTP не примет. Если в пароле встречаются `@ : / ? #`, замените их на
   `%40 %3A %2F %3F %23`.
3. **`DEFAULT_FROM_EMAIL` должен совпадать с адресом входа.** Иначе Яндекс
   отклонит письмо с жалобой на недопустимого отправителя.

После правки перезапустите приложение и отправителя писем:
...
Адреса складываются из двух источников:

- **Общие ящики отделов** — в админке, раздел «Правила маршрутизации заявок».
  Меняются менеджером, учётные записи для этого не нужны.
- **Сотрудники с галочкой «Получать заявки на почту»** — в карточке
  пользователя. Если у сотрудника указан отдел, придут только заявки этого
  отдела; без отдела — все. Так руководитель видит поток целиком, а менеджер
  по запчастям не тонет в заявках на технику.

Галочку нельзя включить без адреса почты — это запрещено на уровне базы:
получатель без адреса означал бы молча теряемые заявки при рабочей на вид
настройке.

#### Демонстрационные адреса и чужая почта

Наполнение `seed_demo` заводит адреса на домене `modernmachinery.example` —
зарезервированном по RFC 2606. Он не резолвится, письмо на него физически не
уйдёт.

**Так было не всегда.** До этого демо-данные использовали настоящий домен
заказчика, и заявка с демонстрационной площадки однажды дошла до реальной
компании. Если ваша база наполнялась ранней версией, вычистите её:

```bash
docker compose -f docker-compose.prod.yml exec web \
  python manage.py purge_email_domain modernmachinery.ru --dry-run
```

Команда показывает, что изменится; без `--dry-run` — применяет. Она убирает
домен отовсюду: из правил маршрутизации, контактов филиала и сотрудников,
почты в настройках сайта. Учётные записи не трогает — адрес там служит
логином, и очистка сломала бы вход; про них команда только предупреждает.

Дополнительно включите предохранитель в `.env`:

```
LEADS_BLOCKED_EMAIL_DOMAINS=modernmachinery.ru
```

Сайт откажется отправлять письма на этот домен, даже если адрес снова окажется
в правиле или его впишут в админке. Проверено: адрес в правиле есть, письмо не
уходит.

**На рабочем сайте компании этот список обязан быть пустым** — иначе он
заблокирует её собственную почту.

Убрать один адрес, не трогая остальное:

```bash
docker compose -f docker-compose.prod.yml exec web \
  python manage.py remove_lead_email sales@example.com
```

Правило при этом остаётся: оно продолжает определять отдел, а письмо уходит
подписчикам с галочкой.

Резервный адрес задаётся в `.env`:

```
LEADS_FALLBACK_EMAIL=вы@yandex.ru
```

На него письмо уходит, только если не нашлось ни ящика в правиле, ни одного
подписчика. Если и он окажется на запрещённом домене, письмо не отправится
вовсе, а в истории заявки останется отметка.

Повторный запуск `seed_demo` списки адресов не перезаписывает — правки
менеджера переживают обновление демонстрационных данных.

Проверить, кому что уйдёт, можно не отправляя заявку с сайта:

```bash
docker compose -f docker-compose.prod.yml exec web \
  python manage.py check_lead_routing
```

Проверить сам факт отправки через SMTP:

```bash
docker compose -f docker-compose.prod.yml exec web \
  python manage.py sendtestemail вы@example.ru
```

Если письмо не пришло, смотрите журнал обработчика задач — отправка идёт
фоном, и ошибка SMTP окажется там:
...
Счётчик подключается, только когда сошлось всё три условия: включён флаг
`METRIKA_ENABLED` (на проде включён по умолчанию), в админке задан номер, и
страницу смотрит не сотрудник. Номер **не хранится в коде и не приезжает с
демо-данными**: он принадлежит заказчику, и демо-стенд с чужим номером слал бы
заходы в его аналитику.

Что сделать после первого запуска:

1. `/admin/` → «Настройки сайта» → «ID Яндекс.Метрики» → только цифры. Счётчик
   появится сразу: сохранение сбрасывает кеш настроек.
2. В кабинете Метрики: Настройки → Вебвизор → **снять «Записывать содержимое
   полей ввода»**. В формах сайта телефон, почта и ИНН — писать их в записи
   сессий нельзя. Сам Вебвизор при этом остаётся включённым.
3. Настройки → Фильтры → добавить IP офиса. Отсечка сотрудников в коде работает
   только для авторизованных, а сайт смотрят и без входа.
4. Часовой пояс счётчика — Хабаровск. По умолчанию встаёт Москва, и отчёт по
   времени суток уезжает на семь часов.

Цели заводятся вручную, тип — «JavaScript-событие». Идентификаторы должны
совпадать с кодом буква в букву, иначе цель молча не засчитается:

| Идентификатор | Когда срабатывает |
|---|---|
| `lead_price` | создана заявка «Запрос цены» |
| `lead_selection` | создана заявка «Подбор техники» |
| `lead_service` | создана заявка в сервис |
| `lead_parts` | создан запрос запчастей |
| `lead_callback` | заказан обратный звонок |
| `lead_vacancy` | отправлен отклик на вакансию |
| `lead_tco` | заявка из калькулятора стоимости владения |
| `lead_leasing` | заявка из калькулятора лизинга |
| `click_phone` | клик по телефону |
| `click_email` | клик по почте |
| `click_whatsapp` | клик по WhatsApp |
| `click_telegram` | клик по Telegram |

Цель отправляется по факту созданной заявки, а не по клику по кнопке: повторная
отправка в течение пяти минут склеивается с прежней заявкой и второй конверсией
не считается.

Метка источника лежит в cookie, подписанной ключом приложения. Смена
`DJANGO_SECRET_KEY` обнуляет её у всех, кто в этот момент в пути: атрибуция по
таким посетителям потеряется. Если ключ приходится менять, старое значение
стоит на время оставить в `SECRET_KEY_FALLBACKS`.

Вебвизор выключается переменной `METRIKA_WEBVISOR=0` без правки кода.

На странице политики конфиденциальности должно быть сказано, что сайт использует
Яндекс.Метрику и собственную cookie для определения источника перехода. Текст
правится в админке, шаблон трогать не нужно.
