...
#### Ошибка `NXDOMAIN looking up TXT for _acme-challenge...`

```
Type: dns
Detail: DNS problem: NXDOMAIN looking up TXT for _acme-challenge.домен
        - check that a DNS record exists for this domain
```

Запись создана, но Let's Encrypt её не увидел. Сначала убедитесь, что домен
действительно обслуживается серверами reg.ru:

```bash
curl -s "https://dns.google/resolve?name=s3raphim-dev.ru&type=NS" | grep -o '"data":"[^"]*"'
```

Должны быть `ns1.reg.ru` и `ns2.reg.ru`. Если там серверы другого провайдера,
плагин пишет запись в зону, которая никем не читается, — нужен плагин под
фактического держателя зоны.

Если серверы верные, причина почти всегда в скорости распространения. reg.ru
разносит изменения по своим серверам имён дольше пяти минут, а **боевой**
сервер Let's Encrypt строже тестового: он проверяет запись с нескольких точек
мира. Поэтому выпуск в тестовом режиме может пройти, а боевой — нет.

Увеличьте ожидание в `.env`:

```
CERTBOT_DNS_PROPAGATION_SECONDS=900
```

При упорных отказах поднимайте до 1800. Посмотреть, видна ли запись публичным
резолверам прямо сейчас, можно так:

```bash
make dns-check
```

Полезно запустить её в соседнем окне, пока идёт выпуск: видно, в какой момент
запись появляется и сколько это заняло.

Ещё один способ сократить риск — отказаться от второго имени. Каждое требует
собственной записи и собственной проверки:
...
до истечения. nginx раз в шесть часов перечитывает конфигурацию и подхватывает
обновлённый сертификат — сам по себе он файлы сертификата не перечитывает, и
без этого посетители получали бы старый.

Вручную — `make tls-renew`, посмотреть срок — `make tls-status`.

### Проверка, что продление действительно сработает

Стоит сделать сразу после выпуска, а не через два месяца:

```bash
docker compose -f docker-compose.prod.yml run --rm \
  --entrypoint "certbot renew --dry-run" certbot
```

Команда проходит весь путь продления на тестовом сервере Let's Encrypt, не
трогая рабочий сертификат и не расходуя лимит. Если подтверждение через DNS
настроено верно, она создаст и удалит временную TXT-запись и отчитается
успехом.
